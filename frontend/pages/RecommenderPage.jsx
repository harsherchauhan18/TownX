...
const RecommenderPage = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Call agent endpoint
      const response = await axios.post("http://localhost:4000/api/agent-search", {
      ...
      const errorMessage = {
        role: "assistant",
        content: `Sorry, I encountered an error: ${error.message}. Make sure the backend is running on localhost:4000`,
      };
      setMessages((prev) => [...prev, errorMessage]);
    } finally {
      setIsTyping(false);
